...
public class StreamService implements IStreamService {
	...
	/**
	 * Close stream. This method can close both IClientBroadcastStream (coming
	 * from Flash Player to Red5) and ISubscriberStream (from Red5 to Flash
	 * Player). Corresponding application handlers (streamSubscriberClose, etc.)
	 * are called as if close was initiated by Flash Player.
	 * 
	 ...
	 * </pre>
	 * 
	 * When stream is closed, corresponding NetStream status will be sent to
	 * stream provider / consumers. Implementation is based on Red5's
	 * StreamService.close()
	 * 
	 * @param connection
	 *            client connection
	...
	public static void closeStream(IConnection connection, int streamId) {
		...
		if (!(stream instanceof IClientBroadcastStream)) {
			StreamService.sendNetStreamStatus(connection, StatusCodes.NS_PLAY_STOP, "Stream closed by server", stream.getName(),
	...
	/**
	 * Pause at given position. Required as "pausePlayback" can be "null" if no
	 * flag is passed by the client
	 * 
	 * @param pausePlayback
	...
	public void pause(Boolean pausePlayback, int position) {
		...
		// pausePlayback can be "null" if "pause" is called without any
